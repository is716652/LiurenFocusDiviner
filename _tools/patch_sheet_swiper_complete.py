# -*- coding: utf-8 -*-
"""补丁：YongShenSheet 迷你盘 → Swiper 三页（动态三传默认 / 天地盘 / 四课）+ kegList + 删下方 DongtaiChuan"""
import io
...
def main():
    ...
    assert s.count(old) == 1, 'mini: %d' % s.count(old)
    new = """          /* 迷你盘预览：Swiper 三页（动态三传默认 / 天地盘 / 四课，左右滑动） */
          if (this.chart && (this.affairName !== '' || this.yongShen !== '')) {
            Column({ space: 4 }) {
              Row() {
                Text('盘面预览')
    ...
    new_m = """  /* 迷你四课数据：chart.kegs → {x:上神, s:下神}（第一课在右） */
...
"""
    ...
    print('SHEET SWIPER COMPLETE OK')
