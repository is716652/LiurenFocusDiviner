 ...
 * ----------------------------------------------------------------------------
 * 按固定顺序跑完整条链（任一步失败即停）：
 *   1) _tools/_ets_facade_extras.js   从 core/liuren/pan/dx.ts 抽 palaceLookup → _ets_extras_dx.txt
 *   2) _tools/_ets_split.js   按「成员→模块」显式映射从基线单体切出各模块 + 生成门面
 *   3) _tools/_ets_qualify.js       归属限定符改写（LiurenCore.X → LrX.X，与 .ts 侧同口径）
...
run('1/4 抽取新增只读接口', process.execPath, [path.join(__dirname, '_ets_facade_extras.js')]);
run('2/4 切片生成 .ets 模块与门面', process.execPath, [path.join(__dirname, '_ets_split.js')]);
run('3/4 归属限定符改写', process.execPath, [path.join(__dirname, '_ets_qualify.js')]);
