...
for (const [caseId, story] of Object.entries(stories)) {
  ...
  /* 待办（2026-09-16）：本应在此加「文案里的课体名必须等于复算课体」判据 ——
     method 锚点写成 {} 时闸门不验课体名，第四批初稿曾因此把两案课体写错而两道闸门全放过。
     我实现过一版但**变异测试未通过**（把「涉害」改成「比用」不报错），已撤掉，
     以免留下"看似在工作、其实不工作"的假门禁；正确实现待下一轮补，并必须先过变异测试。 */
  /* 逐文本核对「文案里的盘面断言」 */
  const checkText = (tag, text) => {
    ...
    /* 煞在支：文本写「<煞名>在|居|临|落<支>」时，核对该支神煞表里是否真有该煞。
       ...
       实现说明（照实现读，别照这段想象）：先按「<2~8 个汉字>在|居|临|落<支>」泛匹配，
       再取**结尾处在煞名表内的最长后缀**作为煞名 —— 因此「本课神煞：日德在卯」这类句子**照样会校验**
       （不存在"句首是本课就跳过"的例外，早先注释如此描述，与实现不符，已更正）。
       之所以不拼大 alternation：煞名含括号（「桃花(咸池)」）会让捕获组错位。 */
