...
class LrDx {
    ...
    static readXiangCard(c, gongOrZhi, yongShenZhi) {
        const rows = [];
        const pl = LrDx.palaceLookup(c, gongOrZhi, yongShenZhi);
        if (pl.gong === "") {
            rows.push(LrDx.cardRow("地盘宫", "该值不是合法地支，无法查宫", "", "warn"));
            return rows;
        }
        /* 此宫：盘上事实（宫 · 天盘支 · 五行阴阳 · 角色） */
        rows.push(LrDx.cardRow("此宫", "地盘" + pl.gong + "宫 · 天盘" + pl.tianZhi + "（" + pl.wuXing + pl.yinYang + "）· " + pl.role.text, "", "gold"));
        /* 远景：旺衰背景（读 wangT()，缺表则说明未加载） */
        const wsAll = LrDx.wangT();
        const wsRow = wsAll[c.r.dg];
        const ws = wsRow === undefined ? "" : (wsRow[pl.tianZhi] || "");
        rows.push(LrDx.cardRow("远景", ws === "" ? "旺衰表未加载：该支旺衰不可用（盘仍照旧排出）" : ("日干" + c.r.dg + "在此支为「" + ws + "」"), "", ws === "" ? "warn" : "plain"));
        ...
        const rel = [];
        const chongZhi = rel0 === undefined || rel0.chong === null ? "" : rel0.chong;
        if (chongZhi !== "") {
            const cnd = c.dx.nodes[chongZhi] || LrDx.EMPTY_NODE;
            rel.push("冲 " + chongZhi + (cnd.kong ? "（该支空）" : ""));
        }
        const heZhi = rel0 === undefined || rel0.he === null ? "" : rel0.he;
        if (heZhi !== "") {
            rel.push("六合 " + heZhi);
        }
        const haiZhi = rel0 === undefined || rel0.hai === null ? "" : rel0.hai;
        if (haiZhi !== "") {
            rel.push("害 " + haiZhi);
        }
        const xingArr = rel0 === undefined ? [] : rel0.xing;
        if (xingArr.length > 0) {
        ...
        const qr = LrDx.qijiReading(c, pl.tianZhi);
        if (qr.gong !== "") {
            const own = [];
            own.push("气机宫位「" + qr.gong + "」（" + qr.side + "）");
            if (qr.chongGong !== "") {
                own.push("冲宫 " + qr.chongGong);
            }
            if (qr.benGongLiuHe !== "") {
                own.push("本宫六合 " + qr.benGongLiuHe);
            }
            if (qr.sanHeJu !== "") {
                own.push("三合局 " + qr.sanHeJu);
            }
            if (qr.kongState !== "") {
            ...
            if (qr.yanChang !== "") {
                src.push(qr.yanChang);
            }
            if (qr.kongNote !== "") {
                src.push(qr.kongNote);
            }
            if (src.length > 0) {
            ...
            rows.push(LrDx.cardRow("气机点", qr.note !== "" ? qr.note : "该支不在日干气机十二宫之内", "", qr.note !== "" ? "warn" : "plain"));
        }
    ...
    static cardRow(label, text, source, tone) {
        const r = {};
        r["label"] = label;
        r["text"] = text;
        r["source"] = source;
...
class LiurenCore {
    ...
    static readXiangCard(c, gongOrZhi, yongShenZhi) { return LrDx.readXiangCard(c, gongOrZhi, yongShenZhi); }
}
