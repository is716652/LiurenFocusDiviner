  ...
  static resolveSanchuan(dg: string, tp: Record<string, string>, kegs: Keg[], dunChuan: Record<string, string>): SanChuan {
    ...
    const sheHai = (list: number[]): string => {
      ...
      if (top.length > 1) {
        ...
        const meng: SheHaiItem[] = top.filter((x: SheHaiItem) => MENG.indexOf(x.gong) >= 0);
        if (meng.length > 0) {
          top = meng;
        } else {
          /* 复等·察微：取所临地盘宫属仲（子午卯酉）者 */
          const zhong: SheHaiItem[] = top.filter((x: SheHaiItem) => ZHONG.indexOf(x.gong) >= 0);
          if (zhong.length > 0) {
            top = zhong;
          }
        }
