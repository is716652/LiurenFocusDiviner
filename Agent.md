...
- **三传按古法竖排（2026-09-26 用户反馈）**：原为**三张卡横排**「初传·中传·末传」，
  用户要求按古法：**初传在下、末传在上**，每行自左至右 **六亲｜天干｜地支｜传位**
  （"迷你盘的三传是古法来排的" —— `壬案推演原型.html` 的三传渲染顺序本就是 `[2,1,0]`，
  即末传在前；App 却是 `[0][1][2]` 正序，与四课「倒序渲染 右起第一课」的口径也不一致）。
  改法：`ChuanCard.ets` 从**竖卡**改为**古法一行**（`Row` 四列固定宽 + 整行居中）；
  三处调用（排盘页／剧情演绎页／旧鉴赏页）由 `Row` 改为 `Column`，按 **[末,中,初]** 倒序渲染。
  `gz` 在组件内拆：**末字恒为地支**（旬空之传无遁干，`gz` 只一字），其前若有字即为天干。
  **Web 原型暂未同步** —— 等这条排法在真机确认后再一次性对齐两个原型，免得方向错了改两遍。

- **「神煞」不另占一列（2026-09-26 用户澄清）**：用户原话"神煞 指的是十二神将，你将玄武等放在了
  地支的下方，其实就不用占神煞这个位置了" —— 即**「神煞」＝十二神将**（玄武、青龙…），
  而神将已作地支的「乘将」写在地支下方，**再单开一列就是重复**。故列结构定为**四列**
  （六亲｜天干｜地支｜传位），原来那一列（旬空标「空」）整列去掉，`ChuanCard` 的 `mark`
  prop 与三处调用/`ChuanRow.mark` 字段一并删除；旬空之传的表现就是**天干列自然留白**（本旬无遁干）。
  **过程留痕（别重犯）**：这一列最初是我为"列对齐"而**去掉了 `if` 条件渲染**，导致非旬空的传也顶着
  一个空色块，用户遂读成"这一传空亡、没有天干"；我先改成"只在有标记时用字色显示"，
  用户随即澄清要**整列去掉**。二审口径：**用户给的列名是他的术语，先问清指的是哪一样东西**，
  再决定摆哪儿 —— "神煞"在六壬语境里指十二神将，不是旬空一类的煞。
  **附**：`AncientCaseGallery.markForZhi`（旧鉴赏页「断句 ↔ 三传」的关键标注）因此**没有调用点**了 ——
  但**未删**：旧鉴赏页只是"不再挂入口"、组件仍在，将来恢复该页时这条联动标注还在原处可直接接回。
...
| **修饰符链里的一句注释，能让对比度门禁静默卸械**（2026-09-26） | 三传改成古法竖排后 `contrast_audit` 报 `ChuanCard.ets:67` 对比度 3.86 —— 但手工核算是 **4.88（合格）**。`--trace` 显示"外层容器背景: []"：工具**推不出底色**，退到保守参考值 `#352F22` 才误判。真因不在嵌套层级（工具设计上就逐层向外找，`for level < 10`），而是 `bgExprsAt`/`sameElementBgExprs` 里 **`if (!trim().startsWith('.')) break;` 撞上注释行就 break** —— `.justifyContent` 上方那句注释把 `.backgroundColor(ink_surface)` 整段切出视野 | **注释行要 `continue` 而不是 `break`**（已修两处）。危害不是"报错"，而是**静默失效**：链上注释之后的背景一律读不到，底色推错或推不出，站点**没有任何提示地失去覆盖**。修完做了变异验证：把地支色改成暗底不达标的 `case_blue_deep` → 报红且底色是 **`#211E18`（即真的 `ink_surface`，证明它确实跨过注释追到了）**，浅主题 0 违规，还原后 sha256 逐字节一致。**判据**：工具报"推不出底色"时，先手工核算真值，再 `--trace` 看它到底缺哪一层 |
