...
**2026-09-22（剧情演绎接入后）全量 47/48 PASS，338.1 s**（门禁有效性自检 283.9 s、组件体检 32.5 s、
行为快照 185,981 条 9.4 s）；快档 **44/44 PASS**（≈10 s）。

唯一红项是 **`verify_app_pkg.py`** 的「rawfile 与主版源码树逐文件对齐」：
它校验的是**免费上架包**（`release_pkg/LiurenFocusDiviner-free-release-signed.app`，仍是 1.0.6 那一版），
而源码此后新增了 `ancient/ouyu_cases.json` 与 `ancient/story_featured.json` ——
报"包内 33 / 期望 35，仅源码有这两个"。**这是"上架包落后于源码"，不是代码缺陷**：
免费版功能一行未变（剧情只在收费版），故**不为它单独出包**；
等你决定出 1.0.7 免费包时，这一项自然转绿。

> 主版侧已出的包是 `release_pkg/LiurenFocusDiviner-main-release-signed.app`（见 §8），
> 与该项检查的对象（免费包）不是同一个包。
