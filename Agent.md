...
> 本文档更新前 main HEAD：`5b94d77 feat(原型): 信息架构重排 —— 剧情为主入口、案卷降为素材地基（收费块定位转向）`（本版为文档改动）
...
| **把 UTF-16 当"乱码坏文件"跳过**（2026-09-22） | 扫古籍语料时按 GBK/UTF-8 打开失败 ⇒ 判定《六壬直指御定》《六壬集成五要权衡》"文件损坏、需处理" ⇒ **前者被整本跳过**，而它其实是 **720 局的断语库**（实测 717 条，UTF-16 完好）。代价：一轮盘点结论全错 | ① **不假定编码**：依次试 `utf-8-sig` → `gb18030` → `utf-16`，取**汉字字符最多**的解码，再判"是不是乱码"；② "解码失败/乱码"**不等于**文件坏；③ 普查方法与结果见 `大六壬文档/案例剧情/素材地基-古籍语料普查与入库管线.md` |
