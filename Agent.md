...
> 最近一轮（换会话前）：补 **§0.1 接手检查清单**（新会话第一件事做什么、开场白可直接粘贴），
...
3. **看两处状态**：`git log --oneline -20`（最近在改什么）＋ §8 发布记录（每一包的指纹与来龙去脉）。
...
2026-09-19 全量 **46/46 PASS**（275.7 s，含 231.6 s 变异自检）—— 即"每个门禁都被证明能抓住它该抓的问题"。
此后新增 `_test_hygiene`（无用 import）与 `_test_page_transition`（转场）两项，
随快档验证（**42 项全过**），并已并入全量重跑（**46/46 全过**，跑完源头逐字节还原）。
...
- **1.0.5 / 1000005（已提审，等待审核结果；2026-09-19 出包）**：`APP/release_pkg/LiurenFocusDiviner-free-release-signed.app`
...
**A. 需要用户（真机 / 目视 / 拍板）** —— 2026-09-19 状态：三项已确认，只剩第 2 条的"杀进程重开"。
...
4. ~~是否提审~~ **已提审，等待审核结果**（2026-09-19）。包：`APP/release_pkg/LiurenFocusDiviner-free-release-signed.app`
   （1,579,759 B / sha256 `75EA0549…AC083`）；本地已打 `v1.0.5-submitted` → **`c1769b2`**
   （**必须打在这一刀**：`v1.0.5-packaged` 标在首刀 `2ba22f3`，那是自检失败那版，不能当本包锚点）。
   上架后补 `v1.0.5-onshelf`。**推送等上架审核通过后随 main 一起做**（用户 2026-09-19 定）。

**B. 我可以继续推进的工程项（按价值排序）**

1. 盘面**逐格**对比度（天支/地盘支/乘将/神煞四层彼此可分辨）目前只覆盖"语义边界 3:1"口径，未逐格验。
2. 跨文件 prop 追溯只覆盖一层数据对象字段；更深的间接会落进"无法解析"清单（会被打印，不会静默）。
