...
1. **1.0.5 第 4 包的真机自检**：状态栏（图标是否可见、与页面是否连续）与转场（是否淡入淡出、无位移、手感）
   都只能真机目视 —— 我无设备，只保证 API 用法正确 + 编译通过 + 门禁卡住了可机械判定的点。
   用户已反馈：**这一包自检通过**。
2. **浅色主题真机目视验证**：① 默认启动是否深色 ② 切换后页面与状态栏是否一起变浅
   ③ 排盘页天地盘是否变浅、四课三传是否外观一致 ④ 杀进程重开是否记住选择。
3. **商店截图的目视核对**（新会话若带视觉可直接看图）：`APP/screenshots_out/1.05/`（gitignored）已出
   1080×1920 深/浅两张，另存了被裁掉的四条条带在 `cropped_off/` 供眼看。需确认：裁掉的确是系统栏、
   没切到软件界面；且截自**免费版**、不含「古籍案例鉴赏」入口（免费包已隐藏该入口）。
4. **是否提审**：包已在 `APP/release_pkg/`；提审后补打 `git tag -a v1.0.5-submitted c1769b2` —— 
   **必须打在 `c1769b2`**（第 4 包源码；`v1.0.5-packaged` 标在首刀 `2ba22f3`，那是自检失败那版，不能当本包锚点）；
   上架后补 `v1.0.5-onshelf`。**推远端等上架审核通过后再做**（用户 2026-09-19 定）。
