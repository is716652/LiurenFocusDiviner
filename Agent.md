...
> - **最近一轮要点**：**「剧情演绎」接入 App（收费块首个落地）** ——
>   古籍页页签「案例鉴赏」换成**「剧情演绎」**，旧鉴赏组件与 45 案数据全留、只撤入口（§11 B 类第 2 条）。
>   上了两个案例、两种体裁：**韩太守占祈雪**（一局多占推演体：3 支线 / 17 线索 / 点盘取证 / 揭古断与应验）、
>   **地铁偶遇**（实盘分层体：7 条取象通道 / L1·L2 分层 / 取象小结 / 「看着最配却不能用」）。
>   新组件 `StoryPlay.ets`；数据侧加 `loadStoryCases`/`loadOuyuCases`，`ouyu_cases.json` 一并导进 App rawfile
>   并列入免费版物理剔除；新增契约测试 `_test_story_play_data.js`（门禁 **48 项**）。
>   已出**主版开发包** `APP/release_pkg/LiurenFocusDiviner-main-release-signed.app`（1,707,020 B，verify-app success）
>   供真机体验。**上架白名单只放了韩太守一案** —— 其余 39 案是统一模板铺的底稿，待逐案重做。
...
> 本文档更新前 main HEAD：`fb43804 feat(剧情演绎): 韩太守案 + 偶遇之占接入 App，收费块入口改为「剧情演绎」`（本版为文档/测试改动）
