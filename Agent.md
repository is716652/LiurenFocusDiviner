...
> - **门禁**：48 项，改完随手跑 `--fast`（44 项 ≈10 s），出包前跑全量（≈5.5 min，须独占）。
...
>   **B 类（我可推进）**：补完古籍剧情最后 4 案 → 按韩太守／偶遇的标准把 39 案底稿逐案重做（§11 B 类）。
> - **最近一轮要点**：**剧情演绎第二轮 —— 「一案一结构」与点盘语义**（详见 §11 B 类第 2 条）：
>   已接入 **4 案 4 种玩法**：知县占失银＝**要素圈**／韩太守占祈雪＝**时线**／郭巡辖占赴任＝**抉择·四问**／
>   邵彦和占身位＝**源流·四段** —— 每种玩法的数据形状各自独立，**不是换皮**（用户的标准："每一关都不一样"）。
>   **5 条「同课异占」支线全部改「自由取象」**：异占**没有古籍原断**（性质是"同一张盘换一个方向看"），
>   硬造一条"正确路径"＝替古人说话 —— 所以只给一份**可取之象**，不设路径、不判对错、不揭古断。
>   并修正**点盘语义**：天地盘是**圆盘**，外圈＝天盘支、内圈＝地盘宫名，而锚点里的乘将/神煞/三传
>   **一律挂天盘支** —— 旧码只比支名、不看层次，"点内圈寅宫"会读到**天盘亥**的象
>   （"第一次选不中、多点几次也能选中＝盲猜"的根因之一）。`anchorHit()` 现按层次判 + 内圈折算重试。
>   **顺手把审计补上去**：`_test_case_story.js` 原先只校验 `clues`、**完全不看 `drama`**，
>   扩到 drama 后**当场抓出知县案 3 处盘面事实错误**（§9）。门禁全量 **48/48 PASS**（388.3 s）；
>   已出主版开发包 `APP/release_pkg/LiurenFocusDiviner-main-release-signed.app`（1,749,045 B，verify-app success）供真机体验。
...
>   其余 39 案是统一模板铺的「一局多占」底稿，**待逐案重做成"为本案量身"的剧情**后再逐个加进来。
> - **上一轮要点**：**古籍语料普查 + 直指御定 717 局对账实测** ——
...
> 本文档更新前 main HEAD：`e196ae8 工具与文档: story_pan 补乘将全表 · Agent.md 记本轮玩法结构/点击口径/两条新坑`
...
**2026-09-22（剧情演绎接入 + 修正包级校验后）全量 48/48 PASS** —— 门禁全绿。
（快档 **44/44 PASS** ≈10 s；全量的耗时构成见下。）
...
**已完成（2026-09-22 收口）**：**1.0.6 已上架**（版本递进与包指纹见 §8）；门禁 **48 项**、全量 48/48 PASS（§3.4）；
...
**A. 需要用户（真机 / 目视 / 拍板）** —— 2026-09-22 状态：**1.0.6 已上架**；三项已确认 + 一条阈值待拍板；
...
7. **剧情演绎的真机试玩**（2026-09-22 新增；主开发包已给，尚未反馈）：重点看三件事 ——
...
2. **✅ 已落地（2026-09-22，`fb43804`）：「剧情演绎」接入 App** —— 收费块入口原「案例鉴赏」页签**换成了「剧情演绎」**，
   ...
   - 上架白名单在 **`rawfile/ancient/story_featured.json`**（**数据驱动**，现只有韩太守一案；
     组件层不得写死个案 id —— 门禁 A1 硬项，所以白名单不进代码）——
     **其余 39 案是按统一模板铺的「一局多占」底稿，待逐案重做成"为本案量身"的剧情后再逐个加进来**。
   ...
   - 注：原型里 `DEFAULT_ENDINGS` 那个从未生效的兜底，App 侧**没有**实现（未用到）；
     原型已清零的小瑕疵不再回填。
   - **玩法结构：一案一结构（2026-09-22 第二轮）** —— 用户的验收话是"如果十几个例子都这个套路，
   ...
   - **点盘语义按层次分（2026-09-22）**：天地盘是**圆盘** —— 外圈给的是**天盘支**、内圈给的是**地盘宫名**；
