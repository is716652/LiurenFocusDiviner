...
- **三传按古法竖排（2026-09-26 用户反馈第二条）**：原为**三张卡横排**「初传·中传·末传」，
  用户要求按古法：**初传在下、末传在上**，每行自左至右 **六亲｜神煞｜天干｜地支｜传位**
  （"迷你盘的三传是古法来排的" —— `壬案推演原型.html` 的三传渲染顺序本就是 `[2,1,0]`，
  即末传在前；App 却是 `[0][1][2]` 正序，与四课「倒序渲染 右起第一课」的口径也不一致）。
  改法：`ChuanCard.ets` 从**竖卡**改为**古法一行**（`Row` 五列固定宽 + 整行居中，
  天将**不入列** —— 它是地支的乘将，作地支列下的小字）；三处调用（排盘页／剧情演绎页／旧鉴赏页）
  由 `Row` 改为 `Column` 并按 **[末,中,初]** 倒序渲染。`gz` 在组件内拆：**末字恒为地支**
  （旬空之传无遁干，`gz` 只一字），其前若有字即为天干。
  顺带把剧情页神煞列补上（原先 `mark: ''` 恒空，与排盘页不一致；现按"旬空→空"同口径）。
  **Web 原型暂未同步** —— 等这条排法在真机确认后再一次性对齐两个原型，免得方向错了改两遍。
  **神煞列（同日紧接修正）**：用户反馈"六亲与神煞之间的红线没必要存在，反而会误认为是空亡、
  没有天干"。根因是我上一刀为求列对齐，把 `mark` 的 `if (mark !== '')` **条件渲染去掉了** ——
  于是**非旬空的传也顶着一个空色块**，看上去就像在指示"这一传空亡"。现恢复条件渲染、
  外层 `Column` 固定宽保对齐，并**改用字色而非底色**（列距窄，色块看着像一条分隔线）；
  字色用 `brand_cinnabar` 而**不是** `state_warn` —— 后者是**底色**令牌（配浅字使用），
  直接当字色时浅主题只有约 3.3:1。
  **口径**：一个标记只在**真有该项**时才出现；"留白"与"没留位"是两回事 —— 前者不误导，后者才对齐不了。
