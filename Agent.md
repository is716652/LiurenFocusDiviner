...
>
> **当前状态（一眼看完，2026-09-21 收口）**
> - **在架**：`1.0.5 / 1000005`（09-19 自检通过 → 提审 → **09-20 正式上架**；1.0.4 同日被取代）。
>   **待提审**：`1.0.6 / 1000006`（**09-21 出包**，含「宫情卡入参口径」修复；包指纹与内容见 §8）。
> - **仓库**：`main` 与 `origin/main` 同步、工作区干净。1.0.5 源码锚点 = `c1769b2`（`v1.0.5-submitted`/`-onshelf` 指它）；
>   1.0.6 源码锚点 = tag `v1.0.6-packaged`（提审后补 `-submitted`、上架后补 `-onshelf`）。
> - **门禁**：46 项，改完随手跑 `--fast`（42 项 ≈10 s），出包前跑全量（≈4 min，须独占）。
> - **待办**：**A 类（需你拍板/确认）** —— ① 浅色主题「杀进程重开后是否记住选择」真机确认；② 天地盘层间可分辨性阈值拍板；
>   ③ 古籍案例鉴赏历批挂账的 4 项待定（§11 A 类第 6 条 → 专文 §10.3）；④ **1.0.6 是否提审**（包已就绪）。
>   **B 类（我可推进）**：补完古籍剧情最后 4 案 → 把「一局多占」推演移植进 App（§11 B 类）。
> - **最近一轮要点**：修**宫情卡入参口径**（点天地盘「天盘」层的支却显示同名地盘宫 → 现按所在宫显示，§9）；
>   修**免费版同步"清空后崩"**（sys.path + 加载期 import，§9）；原型三改（盘上留痕 / 一占多断并置 / 三传成局闭环 /
>   盘面方位改按**地盘十二宫**定位，即上南下北）。
...
- **待提审**：`1.0.6 / 1000006`（2026-09-21 出包，内容要点见 §8；源码锚点 tag `v1.0.6-packaged`）。
  包已在 `APP/release_pkg/`，**是否提审由用户定**：提审后补 `v1.0.6-submitted`、上架后补 `v1.0.6-onshelf`。
- **仓库**：`main` 与 `origin/main` 同步、工作区干净。
...
- **1.0.6 / 1000006（待提审，2026-09-21 出包）**：`APP/release_pkg/LiurenFocusDiviner-free-release-signed.app`
  ...
  **归档命名已按包内版本**，不再出现 09-19 那次"把旧包标成新版本号"的错。
  源码锚点：tag `v1.0.6-packaged`（待提审后补 `-submitted`、上架后补 `-onshelf`）。
