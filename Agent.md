...
>   **已提交审核**：`1.0.6 / 1000006`（**09-21 出包**，含「宫情卡入参口径」修复；包指纹与内容见 §8）。
>   注：提审在 AGC 后台操作、**仓库里留不下痕迹**（`v1.0.6-submitted` 这个 tag 是提审后人工补的，目前**尚未补**），
>   故此处状态来自用户口述；下一版号取决于审核结果，见 §8 与 §11。
> - **仓库**：`main` 领先 `origin/main` **8 个提交**（按用户要求，等 1.0.6 审核有结果再推）、工作区干净。
>   1.0.5 源码锚点 = `c1769b2`（`v1.0.5-submitted`/`-onshelf` 指它）；
...
>   ④ **1.0.6 审核结果**：过了 → 出 1.0.7（断语措辞改写＋偶占）；被打回 → 原地改 1.0.6 重提（专文 §6 执行序）。
...
- **已提交审核**：`1.0.6 / 1000006`（2026-09-21 出包，内容要点见 §8；源码锚点 tag `v1.0.6-packaged`）。
  包已在 `APP/release_pkg/`。**提审在 AGC 后台操作，仓库里留不下痕迹** —— `v1.0.6-submitted` 是提审后人工补的，
  目前**尚未补**，故此状态来自用户口述；结果出来后补 tag 并按 §8 记录。
- **仓库**：`main` 领先 `origin/main` 8 个提交（等 1.0.6 审核有结果再推）、工作区干净。
...
- **1.0.6 / 1000006（已提交审核，2026-09-21 出包）**：`APP/release_pkg/LiurenFocusDiviner-free-release-signed.app`
  ...
  **状态注记**：用户口述已于 2026-09-21 提交上架审核；提审在 AGC 后台操作、仓库内无痕迹，
  故 `v1.0.6-submitted` **尚未补 tag**。审核结果出来后：过了 → 补 `-onshelf` 并按 §6 升 1.0.7；
  被打回 → 原地改 1.0.6 重提（措辞改写与偶占的执行序见 `大六壬文档/案例剧情/断语措辞与合规口径-分级与改写清单.md` §6）。
