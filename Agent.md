...
> - **最近一轮要点**：**古籍语料普查**（两处素材目录、42 个文件）——
>   发现**《六壬直指御定》其实是 720 局的断语库（实测 717 条）**，早先因 UTF-16 被误判为"乱码坏文件"而整本跳过（§9）；
>   普查结论、入库管线提案与 6 项待决策见 `大六壬文档/案例剧情/素材地基-古籍语料普查与入库管线.md`。
>   **本轮未动任何代码**。另：普查后发现工作区 10 个古籍 txt 被**外部**转成 UTF-8（逐字核验相同、**未提交**，待你确认）。
> - **上一轮要点**：原型**信息架构重排**（剧情为主入口、案卷降为素材地基 = 收费块定位转向，`5b94d77`）；
>   修**宫情卡入参口径**（点天地盘「天盘」层的支却显示同名地盘宫 → 现按所在宫显示，§9）；
>   修**免费版同步"清空后崩"**（sys.path + 加载期 import，§9）。
> 本文档更新前 main HEAD：`1ce1e45 chore(语料): 古籍原文统一转 UTF-8（24 个文件，内容逐字不变）`（本版为文档/工具改动）
